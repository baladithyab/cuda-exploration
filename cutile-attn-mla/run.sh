...
echo "=== smoke test (MLA correctness shape) ==="
$PY main.py --smoke

echo
echo "=== bench (DeepSeek-V3 MLA shape) ==="
$PY main.py --bench --csv-out results.csv

echo
echo "=== cubin export for SASS inspection ==="
$PY main.py --export-cubin --cubin-out mla_fwd_fused.cubin
...
if [ -n "$CUOBJ" ]; then
  $CUOBJ --dump-sass mla_fwd_fused.cubin > mla_fwd_fused.sass 2>&1 || true
  HMMA=$(grep -c "HMMA" mla_fwd_fused.sass || true)
  MUFU=$(grep -c "MUFU.EX2" mla_fwd_fused.sass || true)
  echo "SASS lines : $(wc -l < mla_fwd_fused.sass)"
