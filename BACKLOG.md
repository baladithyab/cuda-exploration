...
- 📋 **Wave 20 candidate**: Mojo TC depth. (a) `mojo-matmul-mixed/` — drop to `from std.gpu.compute.mma import mma` and emit `mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32` directly to break the `A.dtype == C.dtype` ceiling. Expected 100-130 TFLOPS, closing 60-80% of cuTile's 159 TF bf16 lead. (b) Add TMA loads to the TF32 path via `std.gpu.host.nvidia.tma`. Expected 70-80 TF, closing the gap to cuTile's 84. (c) Numerical correctness check vs `vendor_blas.matmul`. (d) Optional `mojo-matmul-block-tiled-vectorized/` for the f32 microtile row.
