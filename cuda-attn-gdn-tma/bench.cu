// Wave 22.10 — bench harness for cuda-attn-gdn-tma.
//
...
//
// Includes attn_gdn_tma.cu directly so we share the kernel definition + NPY
// loader. We add cudaEvent timing scaffolding around a single launch at the
// bench shape and emit results.csv.
//
// Build:
...
// Bring in the kernel + helpers. attn_gdn_tma.cu defines main() guarded by
// !ATTN_GDN_TMA_BENCH_HARNESS, so we just include directly.
...
static int run_bench(const std::string& inputs_dir, int warmup, int iters) {
    ...
    printf("[gdn-tma-bench] shape=%s B=%d H=%d d_k=%d d_v=%d  warmup=%d iters=%d\n",
    ...
    printf("[gdn-tma-bench] bytes/iter = %.2f KB\n", bytes_per_iter / 1024.0);
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        if (i < 3 || i == iters - 1) {
            printf("[gdn-tma-bench] iter=%d gpu_ms=%.4f gbps=%.1f\n", i, ms, gbps);
        }
        fprintf(csv, "cuda-attn-gdn-tma,gdn_decode_tma,%d,%d,%d,%d,%d,%.6f,%.3f\n",
    ...
    if (!times.empty()) {
        ...
        printf("\n[gdn-tma-bench] ===== SUMMARY =====\n");
        printf("[gdn-tma-bench] best  gpu_us=%.2f gbps=%.1f\n", best * 1000.0, best_gbps);
        printf("[gdn-tma-bench] mean  gpu_us=%.2f gbps=%.1f\n", mean * 1000.0, mean_gbps);
        printf("[gdn-tma-bench] W1c reference: 417.7 GB/s; cuTile: 610 GB/s; W22.9 async: 245 GB/s\n");
        printf("[gdn-tma-bench] HBM peak (RTX 5090): 1792 GB/s\n");
    }
...
int main(int argc, char** argv) {
    ...
    printf("[gdn-tma-bench] device: %s (sm_%d%d)\n", p.name, p.major, p.minor);
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else { fprintf(stderr, "[gdn-tma-bench] unknown arg %s\n", a.c_str()); return 64; }
    }
