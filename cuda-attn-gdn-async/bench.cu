// Wave 22.9 — bench harness for cuda-attn-gdn-async.
//
// Per task: this cell is COMPILE + CORRECTNESS only. No timed benches in
// W22.9 authoring. The harness is wired so the orchestrator can run it
// serially on idle GPU later (per cuda-exploration session convention).
//
// Build (already wired by Makefile target `bench`):
//   /usr/local/cuda/bin/nvcc -O3 -arch=sm_120 -ccbin clang-14 -o bench bench.cu

#define ATTN_GDN_ASYNC_BENCH_HARNESS 1
...
// Bring in the kernel + helpers. attn_gdn_async.cu also defines main(); guard
// against it by renaming.
#define main attn_gdn_async_unused_main_
#include "attn_gdn_async.cu"
#undef main

// -- bench --
static int run_bench(const std::string& inputs_dir, int warmup, int iters) {
    GDNShape sh{"qwen3_next_decode", 1, 16, 256, 256};
    printf("[gdn-async-bench] shape=%s B=%d H=%d d_k=%d d_v=%d  warmup=%d iters=%d\n",
    ...
    // Bytes/iter — matches flops_gdn.gdn_decode_bytes formula (same as W1c bench).
    const double state_bytes = 2.0 * sh.d_k * sh.d_v * 4.0;       // read + write
    const double io_bytes    = (2.0 * sh.d_k + 2.0 * sh.d_v + 2.0) * 2.0;
    const double bytes_per_iter = (double)B_H * (state_bytes + io_bytes);
    printf("[gdn-async-bench] bytes/iter = %.2f KB\n", bytes_per_iter / 1024.0);
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        if (i < 3 || i == iters - 1) {
            printf("[gdn-async-bench] iter=%d gpu_ms=%.4f gbps=%.1f\n", i, ms, gbps);
        }
        fprintf(csv, "cuda-attn-gdn-async,gdn_decode_async,%d,%d,%d,%d,%d,%.6f,%.3f\n",
    ...
    if (!times.empty()) {
        ...
        printf("\n[gdn-async-bench] ===== SUMMARY =====\n");
        printf("[gdn-async-bench] best  gpu_us=%.2f gbps=%.1f\n", best * 1000.0, best_gbps);
        printf("[gdn-async-bench] mean  gpu_us=%.2f gbps=%.1f\n", mean * 1000.0, mean_gbps);
        printf("[gdn-async-bench] cuTile reference: 610 GB/s (best)\n");
        printf("[gdn-async-bench] W1c   reference:  417.7 GB/s (best)\n");
        printf("[gdn-async-bench] HBM peak (RTX 5090): 1792 GB/s\n");
    }
...
int main(int argc, char** argv) {
    ...
    printf("[gdn-async-bench] device: %s (sm_%d%d)\n", p.name, p.major, p.minor);
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else { fprintf(stderr, "[gdn-async-bench] unknown arg %s\n", a.c_str()); return 64; }
    }
