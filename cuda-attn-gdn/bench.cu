// Wave 17 W1c — bench harness for cuda-attn-gdn.
//
// Per task: this cell is COMPILE + CORRECTNESS only. No timed benches in
// W1c authoring. The harness is wired so the orchestrator can run it
// serially on idle GPU later (per cuda-exploration session convention).
//
// Includes attn_gdn.cu directly so we share the kernel definition + NPY
// loader. We add cudaEvent timing scaffolding around a single launch at
// the bench shape and emit results.csv.
//
// Build (already wired by the Makefile target `bench`):
//   /usr/local/cuda/bin/nvcc -O3 -arch=sm_120 -ccbin clang-14 -o bench bench.cu

#define ATTN_GDN_BENCH_HARNESS 1
...
// Bring in the kernel + helpers. attn_gdn.cu also defines main(); guard against it
// by renaming.
#define main attn_gdn_unused_main_
#include "attn_gdn.cu"
#undef main
...
static int run_bench(const std::string& inputs_dir, int warmup, int iters) {
    ...
    printf("[gdn-bench] shape=%s B=%d H=%d d_k=%d d_v=%d  warmup=%d iters=%d\n",
    ...
    // Bytes/iter — matches flops_gdn.gdn_decode_bytes formula.
    ...
    printf("[gdn-bench] bytes/iter = %.2f KB\n", bytes_per_iter / 1024.0);
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        if (i < 3 || i == iters - 1) {
            printf("[gdn-bench] iter=%d gpu_ms=%.4f gbps=%.1f\n", i, ms, gbps);
        }
        fprintf(csv, "cuda-attn-gdn,gdn_decode_fused,%d,%d,%d,%d,%d,%.6f,%.3f\n",
    ...
    if (!times.empty()) {
        ...
        printf("\n[gdn-bench] ===== SUMMARY =====\n");
        printf("[gdn-bench] best  gpu_us=%.2f gbps=%.1f\n", best * 1000.0, best_gbps);
        printf("[gdn-bench] mean  gpu_us=%.2f gbps=%.1f\n", mean * 1000.0, mean_gbps);
        printf("[gdn-bench] cuTile reference: 610 GB/s (best)\n");
        printf("[gdn-bench] HBM peak (RTX 5090): 1792 GB/s\n");
    }
...
int main(int argc, char** argv) {
    ...
    printf("[gdn-bench] device: %s (sm_%d%d)\n", p.name, p.major, p.minor);
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else { fprintf(stderr, "[gdn-bench] unknown arg %s\n", a.c_str()); return 64; }
    }
